...
def DLIMB00P_validate_fields(record, skip_record):
    ...
    if record['BJFSIF'] == 'N':
        ...
        record['BJFSIF'] = 'N'
    # map Item Group
    if len(record['BJICG']) > 0:
        try:
            record['BJICG'] = item_group[record['BJICG']]
        except KeyError:
            record['BJICG'] = ''    
    # normalize dates
    if record['BJRLSD']:
...
for row in input_rec:
    ...
    #check all fields
    DLIMB00P_validate_fields(output_record, skip_record)
